...
export function getFamilyConstellationPreferences(storage = localStorage) {
  ...
  const selectedProfileIds = Array.isArray(value.selectedProfileIds) ? value.selectedProfileIds.filter((id) => typeof id === "string").slice(0, 6) : [];
...
export function saveFamilyConstellationPreferences(preferences, storage = localStorage) {
  const normalized = {
    selectedProfileIds: [...new Set(preferences.selectedProfileIds ?? [])].slice(0, 6),
