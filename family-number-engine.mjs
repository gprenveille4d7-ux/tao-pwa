export const FAMILY_NUMBER_ENGINE_VERSION = "tao-family-number-1.0.0";
...
export function deriveDateSignature(profile) {
  ...
  return Object.freeze({
    ...
    weekday: new Intl.DateTimeFormat("fr-FR", { weekday: "long", timeZone: "UTC" }).format(new Date(`${profile.birthDate}T00:00:00Z`)),
  });
}
...
function observation({ type, category, participants = [], eventIds = [], values = [], facts = [], calculations = [], score, transformations = 0, clusterKey = null }) {
  ...
  const hash = [...idBasis].reduce((value, char) => ((value * 31 + char.charCodeAt(0)) >>> 0), 2166136261).toString(36);
...
export function analyzeFamilyConstellation({ profiles, events = [], roles = {} }) {
  const safeProfiles = [...new Map((profiles ?? []).filter((profile) => profile?.id && profile?.birthDate).map((profile) => [profile.id, profile])).values()].slice(0, 6);
  ...
  return Object.freeze({
    version: FAMILY_NUMBER_ENGINE_VERSION,
    signatures,
    intervals: buildPairIntervals(signatures),
    ...
    primaryObservations: Object.freeze(clusteredObservations.slice(0, 5)),
    summary: Object.freeze({ total: clusteredObservations.length, strong: clusteredObservations.filter(({ interest }) => interest === LEVELS.high).length, discreet: clusteredObservations.filter(({ interest }) => interest !== LEVELS.high).length }),
  });
}
