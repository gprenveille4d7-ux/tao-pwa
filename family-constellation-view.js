import { analyzeFamilyConstellation } from "./family-number-engine.mjs?v=1.0.1";
import { buildFamilyConstellationReading, familyObservationFacts } from "./family-constellation-semantic.mjs?v=1.0.1";
import {
  createFamilyEventId,
  deleteFamilyEvent,
  FAMILY_EVENT_TYPES,
...
function debugPanel(analysis, selectedProfiles, roles) {
  ...
  details.append(element("pre", { text: JSON.stringify({ profiles: selectedProfiles.map(({ id, firstName }) => ({ id, firstName, role: roles[id] })), derivedValues: analysis.signatures, intervals: analysis.intervals, candidateObservations: analysis.candidates, discardedObservations: analysis.discardedObservations, clusters: analysis.clusters, interestScores: analysis.selectedObservations.map(({ id, interestScore }) => ({ id, interestScore })), selectedObservationIds: analysis.clusteredObservations.map(({ id }) => id), aiObservationIds: analysis.clusteredObservations.slice(0, 16).map(({ id }) => id) }, null, 2) }));
  return details;
}
...
export function createFamilyConstellationModule({ profiles, onAddProfile }) {
  ...
  const render = () => {
    ...
    const defaultIds = profiles.filter(({ relationship }) => ["self", "family", "partner", "child", "parent"].includes(relationship)).map(({ id }) => id).slice(0, 6);
    ...
    profiles.forEach((profile) => {
      ...
      checkbox.addEventListener("change", () => {
        if (checkbox.checked && selectionList.querySelectorAll('input[type="checkbox"]:checked').length > 6) { checkbox.checked = false; return; }
    ...
    selection.addEventListener("submit", (event) => {
      ...
      const analysis = analyzeFamilyConstellation({ profiles: selectedProfiles, events: selectedEvents, roles: selectedRoles });
        ...
        const detail = {
          ...
          contextOptions: {
            ...
            familyConstellation: {
              ...
              observations: displayObservations.map(({ id, type, interest, participantIds, values }) => ({ id, type, interest, participantIds, values })),
            },
          },
        };
