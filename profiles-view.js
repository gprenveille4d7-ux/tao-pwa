...
const RELATIONSHIPS = [
  ["other", "Autre"], ["family", "Famille"], ["friend", "Ami"], ["partner", "Conjoint"],
  ["child", "Enfant"], ["parent", "Parent"],
];
...
function getDayMaster(profile) {
  try {
    ...
    return `${result.dayMaster.elementLabel} ${result.dayMaster.polarity === "yang" ? "Yang" : "Yin"} · ${result.dayMaster.name}`;
  } catch {
    return "À recalculer";
  }
}
...
function facts(profile) {
  ...
  for (const [label, value] of [
    ["Naissance", formatBirthDate(profile.birthDate)], ["Lieu", formatPlace(profile.birthPlace)],
    ["Heure", profile.birthTimeKnown ? profile.birthTime : "Heure inconnue"], ["Maître du Jour", getDayMaster(profile)],
  ]) {
...
function activeCard(profile) {
  ...
  card.append(element("p", { className: "product-eyebrow", text: "PROFIL ACTIF" }), element("h2", { text: profile.firstName }), facts(profile));
  const actions = element("div", { className: "product-actions" });
  const edit = element("button", { className: "product-button product-button--primary", text: "Modifier", attributes: { type: "button" } });
...
function otherProfiles(profiles, activeId) {
  ...
  header.append(element("p", { className: "product-eyebrow", text: "PERSONNES" }), element("h2", { text: "AUTRES PROFILS" }));
  const list = element("div", { className: "profile-list" });
  const others = profiles.filter(({ id }) => id !== activeId);
  if (!others.length) list.append(element("p", { className: "empty-state", text: "Aucune autre personne n’est encore enregistrée." }));
  for (const profile of others) {
    ...
    const use = element("button", { className: "product-button product-button--primary", text: "Utiliser ce profil", attributes: { type: "button" } });
    use.addEventListener("click", () => activate(profile.id));
    const edit = element("button", { className: "product-button product-button--quiet", text: "Modifier", attributes: { type: "button" } });
    edit.addEventListener("click", () => openEditor(profile));
...
function renderPlaceResults(places, list, input, state, status) {
  ...
  status.textContent = places.length ? "Choisis le lieu exact." : "Aucun lieu trouvé.";
}
...
function openEditor(existing = null) {
  ...
  head.append(element("div", { html: `<p class="product-eyebrow">${existing ? "MODIFIER" : "NOUVELLE PERSONNE"}</p><h2 id="profile-editor-title">${existing ? existing.firstName : "Ajouter une personne"}</h2>` }));
  const close = element("button", { className: "product-button product-button--quiet", text: "Fermer", attributes: { type: "button", "aria-label": "Fermer l’éditeur" } });
  close.addEventListener("click", () => panel.remove());
  head.append(close);
  const form = element("form", { className: "profile-form", attributes: { novalidate: "" } });
  const firstName = field(form, { name: "firstName", label: "Prénom", value: existing?.firstName ?? "" });

  if (!existing) {
    const group = element("div", { className: "product-field" });
    const label = element("label", { text: "Lien facultatif", attributes: { for: "profile-relationship" } });
    const select = element("select", { attributes: { id: "profile-relationship", name: "relationship" } });
    for (const [value, text] of RELATIONSHIPS) select.append(element("option", { text, attributes: { value } }));
    group.append(label, select);
    form.append(group);
  }

  const placeGroup = element("div", { className: "product-field product-field--wide" });
  const placeLabel = element("label", { text: "Lieu de naissance", attributes: { for: "profile-place" } });
  ...
  const placeStatus = element("p", { className: "field-status", text: "Saisis au moins trois lettres puis choisis une proposition." });
  ...
  placeInput.addEventListener("input", () => {
    ...
    placeStatus.textContent = "Recherche du lieu…";
    searchTimer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (error.name !== "AbortError") placeStatus.textContent = "La recherche est momentanément indisponible. Réessaie.";
      }
    }, 280);
  });
  ...
  const birthDate = field(form, { name: "birthDate", label: "Date de naissance", type: "date", value: existing?.birthDate ?? "" });
  birthDate.max = new Date().toISOString().slice(0, 10);
  const birthTime = field(form, { name: "birthTime", label: "Heure locale", type: "time", value: existing?.birthTime ?? "", required: existing?.birthTimeKnown !== false });
  ...
  unknownGroup.append(unknown, element("span", { text: "Je ne connais pas l’heure de naissance" }));
  form.append(unknownGroup);

  const error = element("p", { className: "form-error", attributes: { role: "alert" } });
  const save = element("button", { className: "product-button product-button--primary", text: existing ? "Enregistrer les modifications" : "Ajouter cette personne", attributes: { type: "submit" } });
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!name) return void (error.textContent = "Indique un prénom.");
    if (!state.place?.id) return void (error.textContent = "Choisis le lieu exact dans les propositions.");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || date > birthDate.max) return void (error.textContent = "Cette date ne semble pas possible.");
    if (!timeUnknown && !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) return void (error.textContent = "Indique une heure valide ou choisis heure inconnue.");
...
export function renderProfilesView() {
  ...
  const active = getActiveProfile();
  if (!active) {
    root.replaceChildren(element("section", { className: "product-card product-error", text: "Aucun profil valide n’est disponible." }));
    return;
  }
  const pageHeader = element("header", { className: "product-header" });
  pageHeader.append(element("p", { className: "product-eyebrow", text: "PROFILS" }), element("h1", { text: "Les personnes de ton cercle" }), element("p", { className: "product-lead", text: "Chaque personne conserve ses propres données et son propre thème." }));
  const add = element("button", { className: "product-button product-button--add", text: "+ Ajouter une personne", attributes: { type: "button" } });
  add.addEventListener("click", () => openEditor());
  const compare = element("section", { className: "product-card compare-card", attributes: { "aria-disabled": "true" } });
  compare.append(element("div", { html: "<p class=\"product-eyebrow\">COMPARER DEUX PROFILS</p><h2>À venir</h2><p>La compatibilité avancée n’est pas encore activée.</p>" }));
